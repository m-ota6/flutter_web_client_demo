...
function Resampler(fromSampleRate, toSampleRate, channels, inputBuffer) { //resample:再標本化は、ある標本点系列でサンプリングされた信号を、別の標本点系列でサンプリングされた信号に変換すること
    //Input Sample Rate:
    this.fromSampleRate = +fromSampleRate; // 変数前の+: 整数に変換
    //Output Sample Rate:
    this.toSampleRate = +toSampleRate;
    //Number of channels:
    this.channels = channels | 0; // |: ビット論理和
    //Type checking the input buffer:
    if (typeof inputBuffer !== "object") {
        throw(new Error("inputBuffer is not an object."));
    }
    if (!(inputBuffer instanceof Array) && !(inputBuffer instanceof Float32Array) && !(inputBuffer instanceof Float64Array)) { // instanceof: 左のオブジェクトの型が右の型かどうか判定
